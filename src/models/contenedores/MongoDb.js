...
const aux = obj => JSON.parse(JSON.stringify(obj))
...
const main = () => {
  const conexion = process.env.MongoDBpasswordAtlas
...
class MongoDB {
  ...
  async getById (num) {
    try {
      ...
      if (docs) {
        const result = renameField(aux(docs), '_id', 'id')
        return result
      } else {
        return false
      }
